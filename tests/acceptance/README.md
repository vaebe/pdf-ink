...
这里的脚本用 Playwright 驱动真实 Chromium 操作界面，并用 pypdf / PDFium 对导出结果做**独立核对**——
断言的不是「函数被调用了」，而是「导出的 PDF 里确实多了一个大小正确的图片对象、链接注解还在、
离屏页面确实不再占内存」。
...
全部默认值集中在 `harness.mjs`，脚本内不写机器路径。
...
```
tests/acceptance/
├── harness.mjs          共享配置（路径 / 应用地址 / 解释器 / 浏览器启动参数）——脚本一律从这里取值
├── acceptance.mjs       主套件，54 项（A1—A14 + B1—B6）
├── probes/              常备探针（带区分力对照，可独立跑）
├── diagnostics/         一次性诊断脚本（排查历史问题的现场记录，按需运行）
├── fixtures/            9 个夹具 PDF + 3 个生成脚本
└── python/              pypdf / PDFium 独立核对（pdf_probe / render_pdfium / render_pages）
```
...
| 脚本                                          | 断言数 | 用途                                                                                                                              |
| --------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `probes/p2-verify.mjs`                        | 15     | 布局可达性、贴边放置、离屏资源释放；每条都带「旧实现会怎样」的对照                                                                |
| `probes/p3-verify.mjs`                        | 13     | 缩略图切换竞态、保存闸门；含「扰动确实生效」的自证断言                                                                            |
| `probes/p3-control.mjs`                       | —      | 把 P2-4/P2-5 的**根因**回退成旧写法再跑 p3-verify：`apply` / `restore` / `status`。回退后必须变红，否则说明断言恒真或根因判断错了 |
| `probes/thumb-margin-probe.mjs`               | —      | 量缩略图观察器的 `rootMargin` 是否真的生效（**不要按结构类推**，本文件所在仓库的取值经实测才有效）                                |
| `probes/tw-check.mjs`、`probes/tw-visual.mjs` | —      | Tailwind 迁移后的视觉回归核对                                                                                                     |
| `probes/fix-verify.mjs`                       | —      | 历史修复项的针对性复核                                                                                                            |
...
- `tests/acceptance/`（这里）= **可执行的脚本与夹具**，可复现、可审计。
...
3. **测屏幕几何前先等瞬时提示条收掉**。应用把「签名已保存到本地签名库」「已导出 xxx」渲染成**参与布局**
   ...
   跨在拖动过程中就会被误读成「拖动漂移」（见 `harness.mjs` 与 `acceptance.mjs` 的
