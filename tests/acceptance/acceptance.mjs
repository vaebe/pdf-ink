 ...
 * 使用 Playwright 驱动 Chromium 真实操作界面，配合 pypdf / PDFium 做独立核对。
 */
import { execFile } from "node:child_process";
import { createHash } from "node:crypto";
import { access, mkdir, readFile, writeFile } from "node:fs/promises";
import { promisify } from "node:util";
import { chromium, launchOptions, APP, APP_HOST, FIX, OUT_ROOT, PY, PY_DIR } from "./harness.mjs";
...
async function renderPdfium(path, outDir) {
  const { stdout } = await execFileAsync(PY, [`${PY_DIR}/render_pdfium.py`, path, outDir]);
  return stdout.trim();
}
...
try {
  ...
  check(
    ...
    statsBefore.map((item) => `${item.width}x${item.height} 不透明像素=${item.opaque}`).join(" | "),
  );
  check(
    "A1-3 IndexedDB 已落库且尺寸随裁剪变化",
    dbBefore.length === 2 && dbBefore.every((record) => record.bytes > 0),
    dbBefore
      .map((record) => `${record.name} ${record.pixelWidth}x${record.pixelHeight} ${record.bytes}B`)
      .join(" | "),
  );
  ...
  check(
    ...
    statsAfter.map((item) => `${item.width}x${item.height} 不透明像素=${item.opaque}`).join(" | "),
  );
  ...
  await page.waitForFunction(() => {
    const button = [...document.querySelectorAll("[data-testid=signature-pad-dialog] button")].find(
      (item) => item.textContent.includes("清空重写"),
    );
  ...
  const movedDelta = {
    x: Math.hypot(first0.centerX - before[0].centerX - 90, first0.centerY - before[0].centerY + 60),
  };
  ...
  const target = beforeScale.find((item) => Math.abs(item.centerX - (before[0].centerX + 90)) < 40);
  ...
  const renderA9 = await renderPdfium(exportA9.target, `${OUT}/pdfium-a9`);
  note(`A9 PDFium 渲染：${renderA9.split("\n")[0]}`);
  ...
  await renderPdfium(exportA5.target, `${OUT}/pdfium-a5`);
  note(`A5 PDFium 已渲染 5 页 → ${OUT}/pdfium-a5`);
  ...
  for (const scenario of zoomScenarios) {
    ...
    const pageRectBefore = await hidpiPage.locator('.pdf-page[data-page-index="0"]').boundingBox();
    const dx = 70;
    const dy = -40;
    await drag(
      hidpiPage,
  ...
  await openPdf(page, "plain.pdf", 2);
  const fileNameBeforeDecline = (await page.locator("[data-testid=file-name]").innerText()).trim();
  ...
  await page.waitForFunction(
    () =>
      (document.querySelector("[data-testid=banner][data-tone=error]")?.textContent ?? "").length >
      0,
    null,
    { timeout: 20000 },
  );
  ...
  const gap = (value) => (Number.isFinite(value) ? `${value.toFixed(2)}px` : "缺（该实例没放上）");
  check(
