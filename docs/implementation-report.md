...
每次都重跑整套验收，**历史归档结果为 59/59 项通过，且连续两轮逐项结论一致**；该结果对应
6.9 节所述的旧 A16-3 顺序（`Tab → Tab → Shift+Tab → Shift+Tab`）。当前 A16-3 从面板首先执行
`Shift+Tab`，在重新运行整套验收并归档结果前，**调整后的流程仍未验证**。
逐条明细（含每项实测数值）见 `docs/acceptance/acceptance-report.json`
（保留上述历史结果），证据文件与复现方式见 `docs/acceptance/README.md`。
