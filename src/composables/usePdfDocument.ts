...
/** 已被数字签名的文档允许编辑，但必须提示用户其原数字签名会失效。 */
const SIGNED_DOCUMENT_NOTICE =
  "该 PDF 已包含有效的数字签名：继续编辑并导出会使原有数字签名失效。原始文件不会被修改，导出结果会另存为新文件。";
...
export function usePdfDocument() {
  ...
  /**
   * 打开文件。含有效数字签名的文档会先经 `options.confirmSignedDocument` 提示用户，
   * 用户放弃时保持当前会话不变。
   */
